...
struct Field { std::string name; Type type; bool unique=false, primary=false, nullable=true; std::optional<Node> value; };
struct Statement {
    enum Kind {
        select,
        create_table,
...
Statement parse(std::string_view);
Type any_type();
bool scalar_type(const Type&);
bool convertible(const Type& source, const Type& target);
Value convert(Value,const Type&,bool explicit_cast=false);
Value affinity(Value,const Type&);
void install_coercions(Registry&);
void install_string_functions(Registry&);
void install_decimal_coercions(Registry&);
std::optional<Type> decimal_common(std::span<const Type>);
...
struct Lowerer {
    ...
    Expr decimal_literal(const Node&, Expr) const;
...
Result insert(Transaction&, const Statement&, const Registry&, std::span<const Value>);
Result execute(Transaction&, const Statement&, const Registry&, std::span<const Value>);
}
