...
void print(const coresql::Value& value) {
    if (coresql::is_null(value))
        std::cout << "NULL";
    else if (auto n = std::get_if<std::int64_t>(&value))
        std::cout << *n;
    ...
    else if (coresql::type_of(value) == coresql::blobs::type())
        std::cout << "X'" << coresql::blobs::hex(value) << "'";
    else
...
int main(int argc, char** argv) {
    try {
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            if (arg == "--database" && i + 1 < argc)
                ...
                throw std::runtime_error(
                    "usage: coresql_sql_cli [--database path] [script.sql]; otherwise read stdin");
        }
        std::ifstream file;
        if (!script.empty()) {
            file.open(script);
            if (!file)
                throw std::runtime_error("Cannot open SQL script");
        }
        ...
        auto db =
            database.empty() ? coresql::Database(registry) : coresql::Database::open(database, registry);
        coresql::sql::Connection connection(db, registry);
        for (const auto& statement : statements)
            for (const auto& row : connection.execute(statement).rows) {
        ...
        if (connection.in_transaction())
            throw std::runtime_error("Script ended with an uncommitted transaction; changes rolled back");
