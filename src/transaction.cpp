...
void Transaction::Savepoint::rollback() noexcept {
    if (state_ && state_->owner)
        state_->owner->finish_savepoint(state_.get(), true);
}
...
void Transaction::Savepoint::release() noexcept {
    if (state_ && state_->owner)
        state_->owner->finish_savepoint(state_.get(), false);
}
Transaction::Savepoint Transaction::savepoint() {
    active();
    auto state = std::make_shared<SavepointState>(
        SavepointState{this, std::make_unique<detail::State>(*staged_), dirty_, savepoint_});
    savepoint_ = state;
    return Savepoint(std::move(state));
}
void Transaction::finish_savepoint(SavepointState* target, bool restore) noexcept {
    if (restore) {
        staged_ = std::move(target->snapshot);
        dirty_ = target->dirty;
    }
...
void Transaction::retarget_savepoints() noexcept {
    for (auto point = savepoint_; point; point = point->parent)
        point->owner = this;
}
Transaction::Transaction(const std::shared_ptr<detail::Owner>& owner)
    : owner_(owner), base_(owner->capture()), staged_(std::make_unique<detail::State>(*base_)) {
}
...
Transaction::Transaction(Transaction&& other) noexcept
    : owner_(std::move(other.owner_)), base_(std::move(other.base_)), staged_(std::move(other.staged_)),
      dirty_(other.dirty_), savepoint_(std::move(other.savepoint_)) {
    retarget_savepoints();
}
Transaction& Transaction::operator=(Transaction&& other) noexcept {
    if (this != &other) {
        ...
        dirty_ = other.dirty_;
        savepoint_ = std::move(other.savepoint_);
        retarget_savepoints();
    }
    return *this;
}
...
std::shared_ptr<detail::Owner> Transaction::active() const {
    ...
    if (!owner || !staged_)
        throw Error(ErrorCode::state, "Transaction is closed or database no longer exists");
...
void Transaction::commit() {
    ...
    detail::healthy(owner);
    if (savepoint_)
        throw Error(ErrorCode::state, "Release or roll back savepoints before committing");
    if (owner->capture() != base_)
        throw Error(ErrorCode::conflict, "Database changed since transaction began; rollback and retry");
    if (!dirty_) {
        rollback();
        return;
    }
    ...
    rollback();
    // Staged resident chunks are released only by rollback. Cache eviction after
    // that point must never turn an acknowledged commit into an exception.
    if (owner->pager) {
        try {
            owner->pager->trim_to_target();
        } catch (...) {
        }
    }
}
void Transaction::rollback() noexcept {
    if (savepoint_)
        finish_savepoint(nullptr, false);
    staged_.reset();
    base_.reset();
    owner_.reset();
    dirty_ = false;
